...
export default function ConfigModal({
  ...
  const renderSettingsTab = () => (
    <div className="space-y-8 font-sans animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className="space-y-4">
        <p className="text-xs uppercase font-bold text-gray-500 ml-1">Layout</p>
        <div className="p-4 rounded-xl bg-[var(--glass-bg)] border border-[var(--glass-border)]">
            ...
            <div className="flex justify-between text-[10px] text-[var(--text-muted)] font-bold uppercase tracking-wider mt-4">
            <span>0px</span>
            <span>32px</span>
            <span>64px</span>
            </div>
            ...
            <M3Slider
              ...
              onChange={(e) => {
                ...
                try {
                  ...
                  console.error('Failed to save inactivity timeout:', error);
                }
              }}
              colorClass="bg-blue-500"
            />
          ...
          return (
            <div key={update.entity_id} className="p-4 rounded-2xl bg-[var(--glass-bg)] border border-[var(--glass-border)] hover:border-blue-500/50 transition-all">
              <div className="flex items-center gap-4">
                ...
                <button
                  onClick={() => {
                    const [domain, _] = update.entity_id.split('.');
                    if (callService) {
                         callService(domain === 'update' ? 'update' : domain, 'install', { entity_id: update.entity_id });
                    }
                  }}
                ...
                <div className="mt-4 pt-4 border-t border-[var(--glass-border)]">
                  ...
                  {update.attributes?.release_url && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer" 
                      className="text-[10px] text-blue-400 hover:text-blue-300 hover:underline mt-2 inline-flex items-center gap-1 font-bold uppercase tracking-wider"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8" style={{ backdropFilter: 'blur(20px)', backgroundColor: 'rgba(0,0,0,0.3)' }} onClick={handleClose}>
       <style>{`
        ...
        `}</style>
      <div 
        className="w-full max-w-5xl rounded-3xl md:rounded-[3rem] shadow-2xl relative font-sans flex flex-col overflow-hidden backdrop-blur-xl popup-anim bg-[var(--modal-bg)] border-[var(--glass-border)] text-[var(--text-primary)] h-[70vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {isOnboardingActive ? (
