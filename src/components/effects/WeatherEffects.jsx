    ...
    const initParticles = () => {
      ...
      for (let i = 0; i < cfg.count; i++) {
        particles.push({
          ...
          length: cfg.length ? cfg.length.min + Math.random() * (cfg.length.max - cfg.length.min) : 0,
          radius: cfg.radius ? cfg.radius.min + Math.random() * (cfg.radius.max - cfg.radius.min) : 0,
          opacity: Math.random()
        });
      }
    };
